...
public:
    ...
    auto slover(const Trajectory<5, 2>& trajectory) -> std::vector<Eigen::Vector3d> {
        // 1. 获取当前状态
        ...
        for (int k = 0; k <= N; ++k) {
            ...
            if (t_eval > total_duration) t_eval = total_duration;

            Eigen::Vector2d pos = trajectory.getPos(t_eval);
            Eigen::Vector2d vel = trajectory.getVel(t_eval);
        ...
        for (int k = 0; k < N; ++k) {
            double t_eval = t_now + k * dt;
            if (t_eval > total_duration) t_eval = total_duration;
            Eigen::Vector2d vel_w = trajectory.getVel(t_eval);
            Eigen::Vector2d vel_b(c0 * vel_w.x() + s0 * vel_w.y(), -s0 * vel_w.x() + c0 * vel_w.y());
            gradient.segment(3 * (N + 1) + k * 3, 2) = -param_.R.diagonal().head<2>().cwiseProduct(vel_b);
    ...
    for (double t = 0.0; t <= duration + 1e-9; t += step) {
        Eigen::Vector2d p = trajectory.getPos(t);
...
double update_track_time(const Eigen::Vector3d &pose,
    ...
    auto slover(const SplineTrajectory::PPolyND<2, 6>& trajectory) -> std::vector<Eigen::Vector3d> {
        // 1. 获取当前状态
        Eigen::Vector3d x0 = current_pose_;
        const int N = param_.N;
        const double dt = param_.dt;
    ...
    auto get_dt_control() const -> float {
        return param_.dt;
    }
   ...
   * @return 初始游标时间(秒)
   */
    double initialize_track(const Eigen::Vector3d& pose, const SplineTrajectory::PPolyND<2, 6>& trajectory) {
...
private:
    ...
    // 最近的点，更新并返回游标时间
    double update_track_time(const Eigen::Vector3d& pose, const SplineTrajectory::PPolyND<2, 6>& trajectory) {
        const double duration = trajectory.getDuration();
        if (duration <= 0.0) {
            t_track_ = 0.0;
            return 0.0;
